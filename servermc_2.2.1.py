...
<button class="primary" onclick="saveProps()">
💾 Guardar cambios
</button>

<button onclick="resetProps()">
↩ Restablecer
</button>
...
<button onclick="act('whitelist-add')">
Whitelist +
</button>

<button onclick="act('whitelist-remove')">
Whitelist −
</button>
...
<button onclick="act('op')">
Dar OP
</button>

<button onclick="act('deop')">
Quitar OP
</button>
...
document.getElementById('assigned').textContent=data.ram_assigned+' GB';
...
document
.querySelectorAll('#props [data-key]')
.forEach(el=>{
...
document.getElementById('msg').textContent=
'No se pudo enviar el comando STOP.';
                ...
                return self._json(
                    {
                        'ok':True,
                        'message':
                        'Comando STOP enviado. Apagando servidor...'
                    }
                )
...
def iniciar_dashboard():
    ...
    try:
        ...
        print(
            f'{VERDE}🌐 Dashboard: puerto {DASHBOARD_PORT}{RESET}'
        )

    except OSError as e:

        print(
            f'{AMARILLO}⚠️ Dashboard: {e}{RESET}'
        )
